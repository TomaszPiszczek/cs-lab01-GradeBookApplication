            ...
            try
            {
                using (var consoleInputStream = new StringReader("close"))
                {
                    ...
                    using (var consolestream = new StringWriter())
                    {
                        ...
                        Assert.True(output.Contains("command not valid, create requires a name, type of gradebook, if it's weighted (true / false)."), "`GradeBook.UserInterfaces.StartingUserInterface` didn't write 'Command not valid, Create requires a name, type of gradebook, if it's weighted (true / false)..' to the console when the create command didn't contain both a name and type." + output);
            ...
            try
            {
                using (var consoleInputStream = new StringReader("close"))
                {
                    ...
                    using (var consolestream = new StringWriter())
                    {
                        ...
                        Assert.True(output.Contains("standard"), "`GradeBook.UserInterfaces.StartingUserInterface` didn't create a gradebook when the `CreateCommand` included a name, type, and if it was weighted (true / false). output = "+ output);
                    }
                }
            }
            finally
